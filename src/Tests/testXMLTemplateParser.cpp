...
BOOST_AUTO_TEST_CASE(testXMLTemplateParser)
{
  ...
  BOOST_REQUIRE(template203->getInstruction("/MarketTest//Int1", instruction));
  BOOST_CHECK_EQUAL(instruction->getId(), field_id_t("23"));
  BOOST_CHECK_EQUAL(instruction->getName(), "/MarketTest//Int1");
  Codecs::FieldOpCPtr fieldOp;
  BOOST_CHECK(instruction->getFieldOp(fieldOp));
  // ...

  BOOST_REQUIRE(template203->getInstruction("/MarketTest//UInt1", instruction));
  BOOST_CHECK_EQUAL(instruction->getId(), field_id_t());
  BOOST_CHECK_EQUAL(instruction->getName(), "/MarketTest//UInt1");
  BOOST_CHECK(!instruction->getFieldOp(fieldOp));

  BOOST_REQUIRE(template203->getInstruction("/MarketTest//Decimal1", instruction));
  BOOST_CHECK_EQUAL(instruction->getId(), field_id_t());
  BOOST_CHECK_EQUAL(instruction->getName(), "/MarketTest//Decimal1");
  BOOST_CHECK(!instruction->getFieldOp(fieldOp));
  {
